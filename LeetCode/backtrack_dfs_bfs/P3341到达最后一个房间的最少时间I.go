...
// 力扣：https://leetcode.cn/problems/find-minimum-time-to-reach-last-room-i/description/

// 这是一个使用广度优先搜索+优先队列（优先队列的底层是用最小堆来维护的）来解决迷宫最短路径问题的 Go 代码。
// 该算法的目的是计算从网格的起始位置 (0, 0) 移动到目标位置 (n-1, m-1) 所需的最短时间，考虑了每个房间的等待时间。
...
// 					newD = moveTime[x][y] + 1
// 				}
// 				// 如果题目是：3342. 到达最后一个房间的最少时间 II，那么我们只需要更改为下面的代码就可以了：
